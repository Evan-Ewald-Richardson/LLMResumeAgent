...
tensorrt_image = tensorrt_image.apt_install(
    ...
    "faiss-cpu==1.7.4",
    "transformers>=4.38.0", # Ensure transformers version supports Llama3 tokenizer well
    "langchain==0.1.16",   # Or a newer compatible version for text splitting
...
def download_tokenizer_for_build():
    ...
    from transformers import AutoTokenizer # Import inside function scope for build context

    logger.info(f"Downloading tokenizer {MODEL_ID} during image build...")
    try:
        # Just calling this should trigger the download and cache it
        ...
        # Optionally re-raise if tokenizer is absolutely critical for build
        # raise

tensorrt_image = tensorrt_image.run_function(
    download_tokenizer_for_build, # <<< USE THE NAMED FUNCTION HERE
    secrets=[modal.Secret.from_dict({"HF_HUB_ENABLE_HF_TRANSFER": "1"})] # Optional: reuse transfer secret
)
...
MAX_INPUT_LEN = 4096
MAX_OUTPUT_LEN = 4096  
MAX_NUM_TOKENS = 8192  # Keep this the same, as it's the engine constraint
MAX_BATCH_SIZE = 1
...
class ChatRequest(pydantic.BaseModel):
    """Match the request format from the existing frontend"""
    question: str
...
web_image = modal.Image.debian_slim(python_version="3.10").pip_install(
    "numpy<2", # faiss-cpu requires numpy<2
    "fastapi[standard]==0.115.4",
    "pydantic==2.9.2",
    "starlette==0.41.2",
    "sentence-transformers==2.5.1",  # For embeddings
    "faiss-cpu==1.7.4",             # For vector search
)

# Create a data directory in the image
CONTENT_DIR = "/content"

MAX_FILE_TOKENS = 1500 # Threshold for chunking files (adjust as needed)
CHUNK_SIZE = 500      # Target size for chunks if file is too long
CHUNK_OVERLAP = 50    # Overlap between chunks

# Add function to process and embed documents
def setup_knowledge_base():
    """
    Set up the knowledge base. Stores entire file content if token count <= MAX_FILE_TOKENS,
    otherwise chunks the file. Embeds the resulting content (full file or chunks).
    Stores metadata INCLUDING the content snippet for each embedded item.
    """
    import os
    import json
    ...
    logger.info("Running setup_knowledge_base with file grouping logic (v2)...") # Updated log
    ...
    logger.info("Loading embedding model 'all-MiniLM-L6-v2' for setup...")
    ...
    # Stores the final metadata list, including content for each embedded item
    ...
    doc_output_path = os.path.join(container_content_dir, 'documents.json')
    index_output_path = os.path.join(container_content_dir, 'embeddings.faiss')
    model_name_output_path = os.path.join(container_content_dir, 'model_name.txt')
    ...
    if not final_documents_list:
        ...
        with open(model_name_output_path, 'w') as f: f.write('all-MiniLM-L6-v2')
    ...
    # This file might become large if content is large
    ...
    logger.info("Knowledge base setup complete with file grouping logic (v2).")


# Add content copying and RAG setup to the image build
content_source_path = Path(__file__).parent / "content"
tensorrt_image = tensorrt_image.add_local_dir( # Copy local content directory
    local_path=content_source_path,           # Use correct argument name
    remote_path=CONTENT_DIR,                  # Target directory in container
    copy=True
    # REMOVED condition argument
).run_function( # Run setup_knowledge_base directly after copying
    setup_knowledge_base, # Run the main setup function
    secrets=[modal.Secret.from_dict({"SENTENCE_TRANSFORMERS_HOME": "/cache"})] # Provide cache secret
)

class RAGRetriever:
    """Retrieval component for fetching relevant context based on file grouping."""
    def __init__(self):
        # ... (init remains same as previous version, loads documents_metadata) ...
        import json
        import os
        import faiss
        import numpy as np
        ...
        # Define paths
        self.doc_path = os.path.join(CONTENT_DIR, 'documents.json')
        self.model_name_path = os.path.join(CONTENT_DIR, 'model_name.txt')
        self.index_path = os.path.join(CONTENT_DIR, 'embeddings.faiss')
        ...
        try:
            # Load metadata (now includes 'content')
            logger.info(f"Loading documents metadata from: {self.doc_path}")
            with open(self.doc_path, 'r') as f:
                self.documents_metadata = json.load(f) # List of dicts {chunk_id, source_file, ..., content}
            logger.info(f"Loaded metadata for {len(self.documents_metadata)} items.")

            # Load embedding model name and model
            ...
            if len(self.documents_metadata) != self.index.ntotal:
                 logger.warning(f"Metadata count ({len(self.documents_metadata)}) != FAISS index size ({self.index.ntotal}). Check setup.")
    ...
    def get_relevant_context(self, query, initial_k=10, final_k=3):
        """
        Retrieve context. Finds the most relevant file based on initial item retrieval,
        then returns either the full file content or the top chunks from that file.
        Relies on 'content' being stored within self.documents_metadata.
        """
        import numpy as np
        ...
        try:
            ...
            if not retrieved_indices.size: return ""
            ...
            if not was_target_chunked:
                # File was not chunked, the top retrieved item contains the full content
                ...
                # File was chunked, gather top_k chunks specifically from this file
                logger.info(f"Gathering top {final_k} relevant chunks from file '{target_source_file}'.")
                relevant_chunks_content = []
                for idx in retrieved_indices:
                    if len(relevant_chunks_content) >= final_k: break # Found enough
...
@app.cls(
    ...
    secrets=[modal.Secret.from_dict({"SENTENCE_TRANSFORMERS_HOME": "/cache"})] # Add secret here too
)
class ChatModel:
    @modal.enter()
    def load_and_initialize(self):
        """Loads the TRT-LLM engine AND initializes the RAG retriever."""
        import time
        import tensorrt_llm
        ...
        # 1. Initialize TRT-LLM Engine (Existing Logic)
        logger.info("Loading TRT-LLM engine...")
        runner_kwargs = dict(
            engine_dir=f"{ENGINE_DIR}",
            lora_dir=None,
            rank=tensorrt_llm.mpi_rank(),
            max_output_len=MAX_OUTPUT_LEN, # Ensure MAX_OUTPUT_LEN is defined
        )
        self.model = ModelRunner.from_dir(**runner_kwargs)
        logger.info("TRT-LLM engine loaded.")

        # 2. Initialize Tokenizer (Existing Logic)
        ...
        # 3. Initialize RAG Retriever (New Logic)
        logger.info("Initializing RAGRetriever...")
        try:
            # RAGRetriever uses SENTENCE_TRANSFORMERS_HOME from the secret
            self.retriever = RAGRetriever()
            logger.info("RAGRetriever initialized.")
        except Exception as e:
            logger.critical(f"FATAL: Failed to initialize RAGRetriever during startup: {e}", exc_info=True)
            # Prevent the container from starting if RAG fails
            raise RuntimeError("Failed to initialize RAG system") from e

        # 4. Load System Prompt (Existing Logic)
    ...
    def chat_endpoint(self, data: ChatRequestWithRAG) -> ChatResponse:
        ...
        # 1. Retrieve Context using self.retriever
        context = ""
        retrieval_start_time = time.monotonic_ns()
        try:
            # Ensure retriever was initialized (safeguard)
            if not hasattr(self, 'retriever'):
                 logger.error("Retriever not initialized on ChatModel instance!")
                 # Return an error immediately if RAG is critical and missing
                 return ChatResponse(response="Error: Backend RAG component not ready.", error="Retriever missing.")

            context = self.retriever.get_relevant_context(question, final_k=3)
            ...
            # logger.debug(f"Retrieved context: {context[:200]}...") # Optional debug log

        except Exception as e:
            retrieval_duration_ms = (time.monotonic_ns() - retrieval_start_time) / 1e6
            logger.error(f"Error during context retrieval (took {retrieval_duration_ms:.1f}ms): {e}", exc_info=True)
            # Decide whether to return error or proceed without context. Proceeding here.
        ...
        # Define max_new_tokens dynamically (ensure MAX_NUM_TOKENS is defined)
        # Using a reasonable default generation length if space allows
        max_new_tokens = min(1024, MAX_NUM_TOKENS - input_length)
        if max_new_tokens <= 0:
        ...
        # 4. Generate Response using self.model (TRT-LLM)
        ...
        try:
            ...
            # Decode the output (assuming batch size 1)
            # The slicing [0, inputs_t.shape[1]:] might be needed if the model echoes input
            # However, generate usually only returns the new tokens. Let's assume that first.
            # output_ids = outputs_t[0, 0, inputs_t.shape[1]:] # If input is echoed
            output_ids = outputs_t[0, 0] # If only new tokens are returned

            output_text = self.tokenizer.decode(output_ids, skip_special_tokens=False) # Keep special tokens for parsing

            # Extract only the assistant's response
            response = extract_assistant_response(output_text)

            generation_duration_s = (time.monotonic_ns() - generation_start_time) / 1e9
            num_output_tokens = len(output_ids) # Count actual generated token IDs
            ...
            # logger.debug(f"{COLOR['BLUE']}Assistant response: {response[:200]}...{COLOR['ENDC']}") # Optional debug log
